    ...
    def create_wall(self):
        ...
        for row in range(self.rows):
            ...
            for col in range(self.cols):
                ...
                rect = pygame.Rect(block_x, block_y + 100, self.width, self.height)
...
class Paddle():
    def __init__(self, width, height, speed, score, level):
        self.width = width
        self.height = height
    ...
    def reset(self, width, height, speed):
        self.__init__(width, height, speed)
...
# Create a wall
wall = Wall(150, 30, 1, random.randint(6, 10))
wall.create_wall()

# Create a paddle
paddle = Paddle(100, 20, 8, 0, 1)
    ...
    for event in pygame.event.get():
        ...
        if (event.type == pygame.KEYDOWN and (
            ...
            if game_over == 1 and paddle.level <= max_level:
                wall.create_wall()
                paddle = Paddle(100, 20, 8, paddle.score, paddle.level)
            if  paddle.level > max_level:
                MENU = True
                wall.rows = 1
                wall.cols = random.randint(3, 6)
